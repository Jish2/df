...
# tarball contains a leading "$NAME/" dir; --strip-components flattens it
if [ -d "$DEST" ] && [ "$(find "$DEST" -maxdepth 1 -name '*.plist' | wc -l)" -gt 100 ]; then
...
echo "ok: $(find "$DEST" -maxdepth 1 -name '*.plist' | wc -l | tr -d ' ') plists"
